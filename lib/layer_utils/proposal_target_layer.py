...
def proposal_target_layer(rpn_rois, rpn_scores, gt_boxes, _num_classes):
  ...
  """
    ...
    # targets
    # labels: [rois_per_image], rois: [rois_per_image, 5], bbox_targets: [rois_per_image, 4*num_classes]
...
def _get_bbox_regression_labels(bbox_target_data, num_classes):
  ...
  """
    ...
    if inds.numel() > 0:
        ...
        bbox_inside_weights[dim1_inds, dim2_inds] = bbox_targets.new(cfg.TRAIN.BBOX_INSIDE_WEIGHTS).view(-1,
                                                                                                         4).expand_as(
            dim1_inds)

...
from utils.visualization import draw_pred_bounding_boxes
...
class Network(nn.Module):
    ...
    def _add_image(self):
        # add back mean
        image = self._image_summaries['image'] + cfg.PIXEL_MEANS
        # BGR to RGB (opencv uses BGR)
        self._vis_image = image[:, :, :, ::-1].copy(order='C')

    def _add_image_summary(self):
        # use a customized visualization function to visualize the boxes
        self._add_image()
        image = draw_pred_bounding_boxes( \
            self._vis_image,
            self._image_summaries['scores'],
            self._image_summaries['bbox_pred'],
            self._image_summaries['rois'],
            self._image_summaries['im_info'], self._num_classes)

        return tb.summary.image('PREDICTION', torch.from_numpy(image[0].astype('float32') / 255.0).permute(2, 0, 1))
    ...
    def _add_act_summary(self, key, tensor):
        return tb.summary.histogram('ACT/' + key + '/activations', tensor.data.cpu().numpy(), bins='auto'),\
               tb.summary.scalar('ACT/' + key + '/zero_fraction', (tensor.data == 0).float().sum() / tensor.numel())
    ...
    def _anchor_component(self, height, width):
        ...
        :param height: height of feature map
        :param width: width of feature map
        :return: anchors of size (H*W*num_anchors,4)
        '''
    ...
    def _region_proposal(self, net_conv):
        ...
        self._image_summaries['rois'] = rois.data.cpu().numpy()
    ...
    def _region_classification(self, fc7):
        ...
        self._image_summaries['scores'] = cls_prob.data.cpu().numpy()
    ...
    """
        ...
        summaries.append(self._add_image_summary())
        # Add event_summaries
    ...
    def _predict(self, mode):
        ...
        # takes as input h,w of feature map
        ...
        # rois: [rois_per_image, 5], cls_prob: [rois_per_image, num_classes], bbox_pred: [rois_per_image, 4*num_classes]
    ...
    def forward(self, image, im_info, gt_boxes=None, mode='TRAIN'):
        ...
        self._image_summaries['image'] = image
        self._image_summaries['im_info'] = im_info
        ...
        means = bbox_pred.data.new(cfg.TRAIN.BBOX_NORMALIZE_MEANS).repeat(self._num_classes).unsqueeze(0).expand_as(
            bbox_pred)
        self._image_summaries['bbox_pred'] = bbox_pred.mul(Variable(stds)).add(Variable(means)).data.cpu().numpy()

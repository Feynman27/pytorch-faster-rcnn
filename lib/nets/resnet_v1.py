...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1, dilate_first_conv=False):
        ...
        if dilate_first_conv:
            self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, stride=stride, bias=False, dilation=1)  # change
        else:
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.layer4 = self._make_dilation_layer(block, 512, layers[3], stride=1, dilation=2)  # doesn't downsample h,w after roi pooling
        self.avgpool = nn.AvgPool2d(7)
        self.fc = nn.Linear(512 * block.expansion, num_classes)
        # iterate over resnet modules
